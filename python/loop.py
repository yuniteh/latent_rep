...
def loop_cv(raw, params, sub_type, sub=1, train_grp=2, dt=0, feat_type='feat', load=True, noise=True, start_cv=1, max_cv=5, sparsity=True, batch_size=32, latent_dim=4, epochs=30, lr=0.001, train_scale=5, n_train='gauss', mod = 'all',gens=50):
    ...
    if np.sum(ind):
        ...
        for cv in range(start_cv,max_cv):
            filename = create_filename(foldername, cv, sub_type, dt=dt, sub=sub, feat_type=feat_type, latent_dim=latent_dim, epochs=epochs, batch_size=batch_size, n_train=n_train, train_scale=train_scale, lr=lr, sparsity=sparsity)
            if dt == 'cv':
                if mod != 'none':
