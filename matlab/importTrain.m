...
for subs = 1:iter
    ...
    for iDAQFile=1:numfiles(1) %Loop through files (or folders) in DATA/DAQ
        ...
        if strcmp(DAQfilename_Extension,'.DAQ') ==1   % Check file list is in .DAQ format
            ...
            data_out = NaN((size(data_all{1},1)/inc)*length(data_all),size(data_all{1},2),win);
            params_out = NaN((size(data_all{1},1)/inc)*length(data_all),2);
            
            trial_i = 1;
            for trial=1:length(data_all)
                for win_i = 1:inc:size(data_all{trial},1)-win
